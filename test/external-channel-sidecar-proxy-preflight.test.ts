...
import { buildExternalProxyPreflightReceipt } from "../scripts/operator/external-channel-sidecar-external-proxy-preflight.ts";
...
function writeProxyFiles(root: string) {
  writeFile(root, "external_event_proxy.py", "print('proxy')\n");
  writeFile(root, "run_external_proxy.sh", "#!/usr/bin/env bash\n");
  writeFile(root, "send_external_reply.sh", "#!/usr/bin/env bash\n");
...
describe("live sidecar External proxy preflight", () => {
  it("copies bounded legacy proxy files into a non-Desktop runtime", () => {
    const legacyRoot = makeTmpRoot("external-legacy");
    const targetRoot = makeTmpRoot("external-target");
    const outputDir = makeTmpRoot("external-output");
    ...
    expect(fs.existsSync(path.join(targetRoot, "external_event_proxy.py"))).toBe(true);
  ...
  it("blocks Desktop runtime targets", () => {
    ...
    const receipt = buildExternalProxyPreflightReceipt({
      legacyRoot,
      targetRoot: path.join(os.homedir(), "Desktop", "external-proxy-test"),
      outputDir,
      writeRuntime: true,
      generatedAt: "2026-04-27T00:00:00.000Z",
      runSmokeCheck: false,
