...
async function runFuzzer(args: string[] = []) {
  const { stdout } = await execFileAsync(
    process.execPath,
    ["--import", "tsx", "scripts/operator/lcx-external-short-intent-fuzzer.ts", ...args, "--json"],
    {
      cwd: repoRoot,
      env: process.env,
...
describe("lcx-external-short-intent-fuzzer", () => {
  it("proves short External canaries are family-generated and not a fixed whitelist", async () => {
