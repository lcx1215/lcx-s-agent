...
IMAGE_NAME="${OPENCLAW_IMAGE:-lcx:local}"
...
ensure_control_ui_allowed_origins() {
  ...
  current_allowed_origins="$(
    docker compose "${COMPOSE_ARGS[@]}" run --rm lcx-cli \
...
docker compose "${COMPOSE_ARGS[@]}" run --rm --user root --entrypoint sh lcx-cli -c \
...
echo "Gateway token: configured (redacted; stored in $OPENCLAW_CONFIG_DIR/.env or current environment)"
echo "Tailscale exposure: Off (use host-level tailnet/Tailscale setup separately)."
echo "Install Gateway daemon: No (managed by Docker Compose)"
echo ""
docker compose "${COMPOSE_ARGS[@]}" run --rm lcx-cli onboard --mode local --no-install-daemon
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm lcx-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm lcx-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm lcx-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.openclaw.ai/channels"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d lcx-gateway
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if [[ -f "$ROOT_DIR/Dockerfile.sandbox" ]]; then
    echo "Building sandbox image: lcx-sandbox:bookworm-slim"
    docker build \
      -t "lcx-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker lcx-gateway --version >/dev/null 2>&1; then
    ...
    echo "  or use a local build (OPENCLAW_IMAGE=lcx:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --no-deps lcx-cli \
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    if ! docker compose "${BASE_COMPOSE_ARGS[@]}" run --rm --no-deps lcx-cli \
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate lcx-gateway
  fi
else
  ...
  # config alone.
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm lcx-cli \
    config set agents.defaults.sandbox.mode "off" >/dev/null; then
...
echo "Token: configured (redacted)"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f lcx-gateway"
echo "  ${COMPOSE_HINT} exec lcx-gateway node dist/index.js health --token \"\$OPENCLAW_GATEWAY_TOKEN\""
# Bootstraps Docker-based OpenClaw setup for local or server installs.
