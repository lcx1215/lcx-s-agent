...
/**
 * Atomic JSON write for Matrix state files.
 *
 * `writeFileSync` truncates before it writes, so a crash or ENOSPC part way through leaves a
 * half-written file behind — and the things written here (account credentials, sync store meta)
 * are the only copy of that data. Renaming is atomic: a reader sees either the old file or the
