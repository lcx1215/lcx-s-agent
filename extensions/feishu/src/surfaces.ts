...
const CONTROL_ROOM_GROUNDING_CONTRACT_LINES = [
  "[System: Control-room grounding contract: before claiming user-visible-observed, core-verified, started, running, completed, blocked, or unproven, first check the freshest available state evidence instead of answering from chat memory alone.]",
  ...
  "[System: State labels must stay literal: core-verified means local implementation or tests only; user-visible-observed means migrated, built, restarted, probed, and verified through the real Lark/Feishu path; started/running/completed/blocked/unproven must not be blended into one success label.]",
...
const FEISHU_SHARED_STATUS_BOUNDARY_LINES = [
  "[System: Feishu/Lark status boundary: before claiming user-visible-observed, core-verified, started, running, completed, blocked, or unproven, use current evidence instead of chat memory alone.]",
  ...
  "[System: Keep status labels literal across every Feishu/Lark surface: core-verified means local implementation or tests; user-visible-observed means migrated, built, restarted, probed, and verified through the real Lark/Feishu path.]",
