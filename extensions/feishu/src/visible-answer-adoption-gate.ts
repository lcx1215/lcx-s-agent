...
const DIRECT_POSITION_ASK_PATTERN =
  /\b(?:buy|sell|add|reduce|hold|wait|position|sizing|average down|cut loss|stop loss|recover quickly|make.*back)\b|买|卖|加仓|减仓|持有|等待|仓位|持仓|补仓|摊低|摊平|割肉|止损|止盈|回本|快点回本/u;
...
const CHINESE_POSITION_ACTION_PATTERN =
  /(?:应该|建议|可以|不要|别|先别|不建议|先说结论).{0,22}(买|卖|买入|卖出|加仓|减仓|补仓|摊低|摊平|割肉|砍仓|抄底|止损|止盈|持有|等待)/u;

const CHINESE_ACTION_FRAMEWORK_PATTERN =
  /均价策略|止损策略|减亏两条路|抄底|砍仓|摊低成本|快点回本/u;
...
function renderPositionRiskRescueReply(userMessage: string): string {
  ...
  return [
    "这类问题不能直接给交易动作结论。",
    "现在缺：NVDA 组合占比、成本区间、期限、风险预算、最大可承受回撤、最新价格、财报、估值来源和时间戳。",
    "能做的是研究检查：集中度、基本面是否被破坏、估值压缩、利率和流动性压力、技术面仅作观察、亏损后的行为风险。",
