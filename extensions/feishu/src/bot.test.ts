...
describe("buildFeishuAgentBody", () => {
  ...
  it("adds a position-risk research notice for buy or reduce questions", () => {
    ...
    expect(body).not.toContain("Action Triggers");
...
describe("learning council routing", () => {
  it("answers active learning-session status directly from control room", async () => {
    ...
    expect(baseDispatcher.sendFinalReply).toHaveBeenCalledWith({
      text: expect.stringContaining("学习状态"),
    });
    expect(baseDispatcher.sendFinalReply).toHaveBeenCalledWith({
  ...
  it("keeps short learning timeout replies readable for commodity learning requests", async () => {
    ...
    mockLearningCommandApiRoute("API planner selected commodities learning council timeout path");
    mockRunFeishuLearningCouncil.mockReturnValue(new Promise(() => {}));
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-learning-readable-"));
    const cfg: ClawdbotConfig = {
      agents: { defaults: { workspace: tempDir } },
      channels: {
        feishu: {
    ...
    await dispatchMessage({
      ...
      event: {
        sender: { sender_id: { open_id: "ou-user" } },
        message: {
          message_id: "msg-commodity-learning-timeout",
          chat_id: "oc-learning",
          chat_type: "p2p",
          message_type: "text",
          content: JSON.stringify({
  ...
  it("runs the finance learning pipeline directly for concrete market capability learning with a local source", async () => {
    ...
    expect(replyText).toContain("学习内化状态: 已通过验证，可作为研究能力使用");
    expect(replyText).toContain("失败原因: none");
    ...
    expect(replyText).toContain("application validation: 已通过验证，可作为研究能力使用");
    ...
    expect(replyText).toContain("Use retained capability:");
    expect(replyText).toContain("How to apply now:");
    expect(replyText).toContain("Fresh inputs checked or missing:");
    expect(replyText).toContain("Final status: 已通过验证，可作为研究能力使用");
    expect(replyText).toContain(
  ...
  it("fails closed on knowledge internalization audits when no learning receipt proves a retained rule", async () => {
    ...
    expect(replyText).toContain("学习内化状态: 还没证明学进去了");
    expect(replyText).toContain(
      "失败原因: 没有找到金融学习检索回执 (no_finance_learning_retrieval_receipts)",
    );
  ...
  it("answers status boundary audits directly even when the API misclassifies them", async () => {
    const baseDispatcher = {
      sendToolResult: vi.fn(() => false),
      sendBlockReply: vi.fn(() => false),
      sendFinalReply: vi.fn(() => true),
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-status-boundary-"));
    const cfg: ClawdbotConfig = {
      agents: { defaults: { workspace: tempDir } },
      channels: {
        feishu: {
    ...
    const event: FeishuMessageEvent = {
      sender: { sender_id: { open_id: "ou-user" } },
      message: {
        message_id: "msg-status-boundary",
        chat_id: "oc-status",
        chat_type: "p2p",
        message_type: "text",
        content: JSON.stringify({
  ...
  it("answers protocol truth surface asks directly instead of silently dispatching zero replies", async () => {
    ...
    mockCreateGatewayLarkApiRouteProvider.mockReturnValue(async () => ({
      family: "protocol_truth_surface",
      confidence: 0.95,
      rationale: "identity and capability truth surface",
    }));
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-protocol-truth-"));
    const cfg: ClawdbotConfig = {
      agents: { defaults: { workspace: tempDir } },
      channels: {
        feishu: {
    ...
    const event: FeishuMessageEvent = {
      sender: { sender_id: { open_id: "ou-user" } },
      message: {
        message_id: "msg-protocol-truth-simple",
        chat_id: "oc-dm",
        chat_type: "p2p",
        message_type: "text",
        content: JSON.stringify({
          text: "简单验收：现在你是谁？你能做什么？不要讲大话，只列出当前可用能力、不可用边界、下一步会做什么。验收码 lark-simple-identity-test",
        }),
      },
    };
  ...
  it("recognizes Chinese missing-source external learning family asks as source-required truth replies", async () => {
    ...
    mockCreateGatewayLarkApiRouteProvider.mockReturnValue(async () => ({
      family: "external_source_coverage_honesty",
      confidence: 0.9,
      rationale: "Chinese no-link learning request with no concrete source",
    }));
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-source-required-zh-"));
    const cfg: ClawdbotConfig = {
      agents: { defaults: { workspace: tempDir } },
      channels: {
        feishu: {
  ...
  it("does not silently hang when concrete market capability learning points at a missing local source", async () => {
    ...
    expect(replyText).toContain("学习内化状态: 还没开始");
    expect(replyText).toContain("失败原因: local_source_not_found");
  ...
  it("fails closed in the finance learning pipeline without receipts when market capability source cannot be extracted", async () => {
    ...
    expect(replyText).toContain("学习内化状态: 还没开始");
    expect(replyText).toContain("失败原因: finance_article_extraction_gap");
  ...
  it("appends a started timebox status when bounded background learning begins", async () => {
    ...
    expect(baseDispatcher.sendFinalReply).toHaveBeenCalledWith({
      text: expect.stringContaining("限时学习状态"),
    });
    expect(baseDispatcher.sendFinalReply).toHaveBeenCalledWith({
