...
`openclaw.invoke --each`) without adding new LCX Agent code per workflow.
...
This extension depends on LCX Agent internal modules (the embedded agent runner).
It is intended to ship as a **bundled** LCX Agent extension (like `lobster`) and
