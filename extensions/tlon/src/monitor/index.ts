import type { RuntimeEnv, ReplyPayload, OpenClawConfig } from "lcx-agent/plugin-sdk";
import { createLoggerBackedRuntime, createReplyPrefixOptions } from "lcx-agent/plugin-sdk";
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in LCX Agent config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your LCX Agent config:\n` +
