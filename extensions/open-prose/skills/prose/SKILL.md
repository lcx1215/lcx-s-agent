...
- **Task tool** in the upstream spec == LCX Agent `sessions_spawn`
- **File I/O** == LCX Agent `read`/`write`
- **Remote fetch** == LCX Agent `web_fetch` (or `exec` with curl when POST is required)
