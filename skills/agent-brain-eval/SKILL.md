...
1. Check the current owner evidence with `node --import tsx scripts/dev/lcx-context-recovery-exam.ts --json` and `node --import tsx scripts/dev/local-brain-training-plan.ts --json`.
