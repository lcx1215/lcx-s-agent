---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for LCX Agent deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, LCX Agent cron scheduling for periodic checks, or version status checks on a machine running LCX Agent (laptop, workstation, Pi, VPS).
---

# LCX Agent Host Hardening

## Overview

Assess and harden the host running LCX Agent, then align it to a user-defined risk tolerance without breaking access. Use LCX Agent security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim LCX Agent changes the host firewall, SSH, or OS updates; it does not.
...
5. LCX Agent gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if LCX Agent can access sensitive data.
...
### 2) Run LCX Agent security audits (read-only)

As part of the default read-only checks, run `lcx security audit --deep`. Only offer alternatives if the user requests them:

1. `lcx security audit` (faster, non-probing)
2. `lcx security audit --json` (structured output)

Offer to apply LCX Agent safe defaults (numbered):

1. `lcx security audit --fix`

Be explicit that `--fix` only tightens LCX Agent defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check LCX Agent version/update status (read-only)

As part of the default read-only checks, run `lcx update status`.
...
- Credential hygiene notes (location of LCX Agent creds, prefer disk encryption)
...
- LCX Agent security audit (re-run)
...
After LCX Agent install or first hardening pass, run at least one baseline audit and version check:

- `lcx security audit`
- `lcx security audit --deep`
- `lcx update status`

Ongoing monitoring is recommended. Use the LCX Agent cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `lcx cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `lcx update status`
...
Before creating, `lcx cron list` and match on exact `name`. If found, `lcx cron edit <id> ...`.
If not found, `lcx cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `lcx update status` (preferred for source checkouts and channels)
2. `npm view lcx version` (published npm version)

## LCX Agent command accuracy

Use only supported commands and flags:

- `lcx security audit [--deep] [--fix] [--json]`
- `lcx status` / `lcx status --deep`
- `lcx health --json`
- `lcx update status`
- `lcx cron add|list|runs|run`

Do not invent CLI flags or imply LCX Agent enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by LCX Agent compaction:
