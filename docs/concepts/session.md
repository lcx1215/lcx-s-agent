...
All session state is **owned by the gateway** (the “master” OpenClaw). UI clients (WebChat, Control UI, TUI, etc.) must query the gateway for session lists and token counts instead of reading local files.
