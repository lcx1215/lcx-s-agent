...
If you do nothing, LCX Agent runs a single agent:
...
```bash
lcx agents add coding
lcx agents add social
```
...
- If you later add the same binding for the same agent with an explicit account id, LCX Agent upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, LCX Agent falls back
...
```bash
lcx channels login --channel whatsapp --account personal
lcx channels login --channel whatsapp --account biz
```
