---
title: "Memory"
summary: "How LCX Agent memory works (workspace files + automatic memory flush)"
read_when:
...
LCX Agent memory is **plain Markdown in the agent workspace**. The files are the
...
LCX Agent exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, LCX Agent triggers a **silent,
...
LCX Agent can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, LCX Agent auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. LCX Agent
...
BM25 + vectors + reranking. Markdown stays the source of truth; LCX Agent shells
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, LCX Agent retries with `qmd query`. If QMD fails or the binary is
  missing, LCX Agent automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- LCX Agent does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - LCX Agent sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index LCX Agent
    uses), run a one-off query with the agent’s XDG dirs.

    LCX Agent’s QMD state lives under your **state dir** (defaults to `~/.openclaw`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    LCX Agent uses:

    ```bash
    # Pick the same state dir LCX Agent uses
...
- When `scope` denies a search, LCX Agent logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, LCX Agent exports sanitized session
...
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, LCX Agent automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, LCX Agent combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, LCX Agent falls back to vector-only search.
...
LCX Agent can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
When the sqlite-vec extension is available, LCX Agent stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, LCX Agent logs the
