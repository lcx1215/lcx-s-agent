...
- `/usage cost` in chats: local cost summary aggregated from LCX Agent session logs.
- CLI: `lcx status --usage` prints a full per-provider breakdown.
- CLI: `lcx channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
