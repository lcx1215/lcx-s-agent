---
summary: "Run LCX Agent in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want LCX Agent isolated from your main macOS environment
...
# LCX Agent on macOS VMs (Sandboxing)
...
Run LCX Agent in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
5. SSH in, install LCX Agent, configure channels
6. Done
...
The VM runs in the background. LCX Agent's daemon keeps the gateway running.

To check status:

```bash
ssh youruser@192.168.64.X "lcx status"
```
...
This is the killer feature of running on macOS. Use [BlueBubbles](https://bluebubbles.app) to add iMessage to LCX Agent.
...
| WhatsApp QR not scanning | Ensure you're logged into the VM (not host) when running `lcx channels login` |
