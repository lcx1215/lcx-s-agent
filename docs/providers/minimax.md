---
summary: "Use MiniMax M2.5 in LCX Agent"
read_when:
  - You want MiniMax models in LCX Agent
...
- **Compatibility:** LCX Agent still accepts legacy `MiniMax-M2.5-Lightning` configs, but prefer
...
```bash
lcx plugins enable minimax-portal-auth  # skip if already loaded.
lcx gateway restart  # restart if gateway is already running
lcx onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/lcx1215/lcx-s-agent/tree/main/extensions/minimax-portal-auth) for details.
...
- Use `lcx models list` and `lcx models set minimax/MiniMax-M2.5` to switch.
...
- Running `lcx configure` and selecting **MiniMax M2.5**, or
