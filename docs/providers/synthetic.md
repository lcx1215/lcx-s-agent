---
summary: "Use Synthetic's Anthropic-compatible API in LCX Agent"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. LCX Agent registers it as the
...
Note: LCX Agent's Anthropic client appends `/v1` to the base URL, so use
