---
summary: "Use OpenAI via API keys or Codex subscription in LCX Agent"
read_when:
  - You want to use OpenAI models in LCX Agent
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like LCX Agent.
...
```bash
lcx onboard --auth-choice openai-api-key
# or non-interactive
lcx onboard --openai-api-key "$OPENAI_API_KEY"
```
...
lcx onboard --auth-choice openai-codex

# Or run OAuth directly
lcx models auth login --provider openai-codex
```
...
LCX Agent uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), LCX Agent also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. LCX Agent enables it by default for
...
`baseUrl` on `api.openai.com`), LCX Agent now auto-enables OpenAI server-side
