---
summary: "Use Venice AI privacy-focused models in LCX Agent"
read_when:
  - You want privacy-focused inference in LCX Agent
...
```bash
lcx onboard --auth-choice venice-api-key
```
...
```bash
lcx agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, LCX Agent shows all available Venice models. Pick based on your needs:
...
```bash
lcx models set venice/claude-opus-45
lcx models set venice/llama-3.3-70b
```

List all available models:

```bash
lcx models list | grep venice
```

## Configure via `lcx configure`
...
LCX Agent automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
lcx agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
lcx agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
lcx agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
lcx agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
lcx agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `lcx models list` to see currently available models. Some models may be temporarily offline.
