...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a LCX Agent Gateway.
...
it to drive a LCX Agent Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `lcx acp` over stdio.
...
- `lcx acp client` sets `OPENCLAW_SHELL=acp-client` on the spawned bridge process.
