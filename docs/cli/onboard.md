---
summary: "CLI reference for `lcx onboard` (interactive onboarding wizard)"
read_when:
...
```bash
lcx onboard
lcx onboard --flow quickstart
lcx onboard --flow manual
lcx onboard --mode remote --remote-url wss://gateway-host:18789
```
...
- Fastest first chat: `lcx dashboard` (Control UI, no channel setup).
