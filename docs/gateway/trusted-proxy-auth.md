...
read_when:
  - Running LCX Agent behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of LCX Agent
...
- You run LCX Agent behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. LCX Agent checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. LCX Agent extracts the user identity from the configured header
...
- LCX Agent can stay on loopback HTTP behind the proxy.
...
If LCX Agent itself serves HTTPS directly (no TLS-terminating proxy), set:
...
`lcx security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update LCX Agent config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `lcx security audit` and review findings
