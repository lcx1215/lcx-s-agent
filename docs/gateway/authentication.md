...
LCX Agent supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `lcx gateway`).
...
API keys for daemon use: `lcx onboard`.
...
```bash
lcx models auth paste-token --provider anthropic
lcx models auth paste-token --provider openrouter
```
...
- LCX Agent retries with the next key only for rate-limit errors (for example
...
Run `lcx models status` to confirm which profile is expiring. If the profile
