...
LCX Agent can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. LCX Agent requires a shared password.
...
(`tailscale-user-login`) without supplying a token/password. LCX Agent verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
LCX Agent only treats a request as Serve when it arrives from loopback with
...
- Set `gateway.tailscale.resetOnExit` if you want LCX Agent to undo `tailscale serve`
