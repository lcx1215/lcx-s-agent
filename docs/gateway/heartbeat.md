...
- During heartbeat runs, LCX Agent treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `lcx sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, LCX Agent skips the heartbeat run entirely (no model call).
...
```bash
lcx system event --text "Check for urgent follow-ups" --mode now
```
