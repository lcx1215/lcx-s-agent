...
> LCX Agent is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
LCX Agent security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `lcx security audit`
...
lcx security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

LCX Agent is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
LCX Agent assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.openclaw`, including `openclaw.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- LCX Agent can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want LCX Agent, use one VPS/host per user.
...
LCX Agent is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/lcx1215/lcx-s-agent/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, LCX Agent also attempts a best-effort live Gateway probe.
...
| -------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full LCX Agent state                                | filesystem perms on `~/.openclaw`                                                                 | yes      |
...
`lcx security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`lcx security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in LCX Agent config
schema:
...
- LCX Agent gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from LCX Agent responses.
...
LCX Agent stores session transcripts on disk under `~/.openclaw/agents/<agentId>/sessions/*.jsonl`.
...
LCX Agent can refresh the skills list mid-session:
...
- If you install plugins from npm (`lcx plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.openclaw/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
  - LCX Agent uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
By default, LCX Agent routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
LCX Agent has two separate “who can trigger me?” layers:
...
LCX Agent includes explicit bypass flags that disable external-content safety wrapping:
...
`lcx doctor` can warn and offer to tighten these permissions.
...
If you run LCX Agent with Docker on a VPS, remember that published container ports
...
Doctor can generate one for you: `lcx doctor --generate-gateway-token`.
...
2. Restart the Gateway (`lcx gateway restart`, or restart the service).
3. Update any remote clients (`gateway.remote.token` / `.password` on machines that call into the Gateway).
4. Verify you can no longer connect with the old credentials.

### 0.6) Tailscale Serve identity headers

When `gateway.auth.allowTailscale` is `true` (default for Serve), LCX Agent
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. LCX Agent verifies the identity by resolving the
...
- LCX Agent will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
- When sharing diagnostics, prefer `lcx status --all` (pasteable, secrets redacted) over raw logs.
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only LCX Agent clients can connect.
- Disable browser proxy routing when you don’t need it (`gateway.nodes.browser.mode="off"`).
- Chrome extension relay mode is **not** “safer”; it can take over your existing Chrome tabs. Assume it can act as you in whatever that tab/profile can reach.

### Browser SSRF policy (trusted-network default)

LCX Agent’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default LCX Agent limits these tools
...
1. **Stop it:** stop the Gateway service (`lcx gateway stop`) or terminate your `lcx gateway` process.
...
4. Re-run `lcx security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + LCX Agent version
...
Found a vulnerability in LCX Agent? Please report responsibly:

1. Report privately: [GitHub security advisories](https://github.com/lcx1215/lcx-s-agent/security/advisories/new)
