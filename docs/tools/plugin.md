---
summary: "LCX Agent plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends LCX Agent with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core LCX Agent yet (or you want to keep optional features out of your main
install).
...
LCX Agent plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
4. Bundled extensions (shipped with LCX Agent, **disabled by default**)

- `<openclaw>/extensions/*`

Bundled plugins must be enabled explicitly via `plugins.entries.<id>.enabled`
or `lcx plugins enable <id>`. Installed plugins are enabled by default,
but can be disabled the same way.

Hardening notes:

- If `plugins.allow` is empty and non-bundled plugins are discoverable, LCX Agent logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. LCX Agent blocks candidates when:
...
Security note: `lcx plugins install` installs plugin dependencies with
...
LCX Agent can also merge **external channel catalogs** (for example, an MPM
...
If a plugin exports `id`, LCX Agent uses it but warns when it doesn’t match the
configured id.
...
LCX Agent augments `uiHints` at runtime based on discovered plugins:
...
lcx plugins install ./extensions/voice-call # relative path ok
lcx plugins install ./plugin.tgz           # install from a local tarball
lcx plugins install ./plugin.zip           # install from a local zip
lcx plugins install -l ./extensions/voice-call # link (no copy) for dev
...
If stored integrity metadata changes between updates, LCX Agent warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `lcx voicecall`).
...
- Plugin-managed hooks show up in `lcx hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `lcx hooks`; enable/disable the plugin instead.

## Provider plugins (model auth)

Plugins can register **model provider auth** flows so users can run OAuth or
API-key setup inside LCX Agent (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `lcx models auth login --provider <id> [--method <id>]`
...
- `config`: The current LCX Agent config
...
- CLI: `lcx voicecall start|status`
- Tool: `voice_call`
