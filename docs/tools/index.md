---
summary: "Agent tool surface for LCX Agent (browser, canvas, nodes, message, cron) replacing legacy `openclaw-*` skills"
read_when:
...
LCX Agent exposes **first-class agent tools** for browser, canvas, nodes, and cron.
...
- If `tools.allow` only references unknown or unloaded plugin tool names, LCX Agent logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:openclaw`: all built-in LCX Agent tools (excludes provider plugins)
...
- `host=node` targets a headless node host (`lcx node run`).
...
LCX Agent tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `lcx configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated LCX Agent-managed browser.
...
- Quick smoke: `lcx nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- Camera/screen commands require the node client to be foregrounded.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `lcx gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, LCX Agent uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, LCX Agent clamps `tools.sessions.visibility` to `tree`.
