---
summary: "Chrome extension: let LCX Agent drive your existing Chrome tab"
read_when:
...
The LCX Agent Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openclaw-managed Chrome profile.
...
LCX Agent then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the LCX Agent release (npm package) as static files. There is no separate “build” step.

After upgrading LCX Agent:

- Re-run `lcx browser extension install` to refresh the installed files under your LCX Agent state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

LCX Agent ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want LCX Agent to control.
...
- `ON`: attached; LCX Agent can drive that tab.
...
`lcx browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `lcx browser extension install` first to copy the extension to a stable location under your LCX Agent state directory.
