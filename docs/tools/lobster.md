---
title: Lobster
summary: "Typed workflow runtime for LCX Agent with resumable approval gates."
description: Typed workflow runtime for LCX Agent — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets LCX Agent run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: LCX Agent runs one Lobster tool call and gets a structured result.
...
LCX Agent launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In LCX Agent tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the LCX Agent Gateway (see the [Lobster repo](https://github.com/openclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), LCX Agent keeps core tools enabled. To restrict core
...
→ lcx calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ lcx calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls LCX Agent tools that do.
