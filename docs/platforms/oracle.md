---
summary: "LCX Agent on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up LCX Agent on Oracle Cloud
  - Looking for low-cost VPS hosting for LCX Agent
  - Want 24/7 LCX Agent on a small server
title: "Oracle Cloud"
---

# LCX Agent on Oracle Cloud (OCI)

## Goal

Run a persistent LCX Agent Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for LCX Agent (especially if you already have an OCI account), but it comes with tradeoffs:
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `lcx security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Security audit:** `lcx security audit`
