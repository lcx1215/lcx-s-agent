...
macOS can also run in **node mode**: the node host connects to the Gateway’s WS server and exposes local canvas/camera commands as a node (so `lcx nodes …` works against this Mac).
...
- `node.pair.*` (CLI: `lcx nodes pending/approve/reject`) is a separate gateway-owned
...
- `lcx node run` reads `OPENCLAW_GATEWAY_TOKEN` for auth.
...
- `lcx nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
lcx nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
...
```bash
lcx nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
lcx nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
lcx nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
- `system.run` is gated by exec approvals (`~/.openclaw/exec-approvals.json`).
  Ask/allowlist/full behave the same on every host; denied prompts return `SYSTEM_RUN_DENIED`.
...
LCX Agent can run a **headless node host** (no UI) that connects to the Gateway
...
- The headless node host executes `system.run` locally.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The node host connects to the Gateway WS server as a node (so `lcx nodes …` works against this Mac).
- In remote mode, forward the Gateway port over SSH and point the node host at `localhost`.
