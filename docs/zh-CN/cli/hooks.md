...
read_when:
  - 你想管理智能体钩子
  - 你想安装或更新钩子
summary: CLI 参考：`lcx hooks`（智能体钩子）
title: hooks
x-i18n:
...
```bash
lcx hooks enable <name>
```

通过将特定钩子添加到配置（`~/.openclaw/config.json`）来启用它。

**注意：** 由插件管理的钩子在 `lcx hooks list` 中显示 `plugin:<id>`，
...
- 重启 Gateway 网关以重新加载钩子（`lcx gateway restart`）。

## 禁用钩子

```bash
lcx hooks disable <name>
```
...
```bash
# 本地目录
lcx hooks install ./my-hook-pack

# 本地压缩包
lcx hooks install ./my-hook-pack.zip

# NPM 包
openclaw hooks install @openclaw/my-hook-pack

# 链接本地目录而不复制
lcx hooks install -l ./my-hook-pack
```
