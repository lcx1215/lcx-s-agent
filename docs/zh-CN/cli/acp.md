...
运行与 LCX Agent Gateway 网关通信的 ACP（Agent Client Protocol）桥接器。
...
lcx acp --session agent:main:main --reset-session
```
...
lcx acp client --server-args --url wss://gateway-host:18789 --token <token>

# Override the server command (default: openclaw)
lcx acp client --server "node" --server-args openclaw.mjs acp --url ws://127.0.0.1:19001
```

## 如何使用

当 IDE（或其他客户端）使用 Agent Client Protocol 并且你希望它驱动 LCX Agent Gateway 网关会话时，请使用 ACP。

1. 确保 Gateway 网关正在运行（本地或远程）。
2. 配置 Gateway 网关目标（配置或标志）。
3. 将你的 IDE 配置为通过 stdio 运行 `lcx acp`。
...
在 Zed 中，打开 Agent 面板并选择"LCX Agent ACP"来开始一个会话。
