...
read_when:
  - 安装 LCX Agent
  - 你想从 GitHub 安装
summary: 安装 LCX Agent（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
- 运行新手引导：`lcx onboard --install-daemon`
- 快速检查：`lcx doctor`
- 检查 Gateway 网关健康状态：`lcx status` + `lcx health`
- 打开仪表板：`lcx dashboard`
...
```bash
# 显式 npm
curl -fsSL https://raw.githubusercontent.com/lcx1215/lcx-s-agent/main/scripts/install.sh | bash -s -- --install-method npm
