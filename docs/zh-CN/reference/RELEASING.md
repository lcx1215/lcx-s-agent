...
- **npm pack/publish 挂起或产生巨大 tarball**：意外的构建输出被扫入包中。通过 `package.json` 的 `files` 白名单发布内容来修复（包含 dist 子目录、docs、skills）。用 `npm pack --dry-run` 确认只列出了预期路径。
