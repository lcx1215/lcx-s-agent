...
summary: LCX Agent 的智能体工具接口（browser、canvas、nodes、message、cron），替代旧版 `openclaw-*` Skills
title: 工具
x-i18n:
...
LCX Agent 为 browser、canvas、nodes 和 cron 暴露**一流的智能体工具**。
...
- 匹配不区分大小写。
- 支持 `*` 通配符（`"*"` 表示所有工具）。
- 如果 `tools.allow` 仅引用未知或未加载的插件工具名称，LCX Agent 会记录警告并忽略允许列表，以确保核心工具保持可用。
...
- `group:nodes`：`nodes`
- `group:openclaw`：所有内置 LCX Agent 工具（不包括提供商插件）
...
- `host=node` 针对无头节点主机（`lcx node run`）。
...
- 需要 Brave API 密钥（推荐：`lcx configure --section web`，或设置 `BRAVE_API_KEY`）。
...
- 快速冒烟测试：`lcx nodes canvas a2ui push --node <id> --text "Hello from A2UI"`。
...
- `restart`（授权 + 发送 `SIGUSR1` 进行进程内重启；`lcx gateway` 就地重启）
