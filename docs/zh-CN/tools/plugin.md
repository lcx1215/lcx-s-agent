...
read_when:
  - 添加或修改插件/扩展
  - 记录插件安装或加载规则
summary: LCX Agent 插件/扩展：发现、配置和安全
title: 插件
x-i18n:
...
插件只是一个**小型代码模块**，用额外功能（命令、工具和 Gateway 网关 RPC）扩展 LCX Agent。

大多数时候，当你想要一个尚未内置到核心 LCX Agent 的功能（或你想将可选功能排除在主安装之外）时，你会使用插件。
...
LCX Agent 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不会执行插件代码**；它使用插件清单和 JSON Schema。参见 [插件清单](/plugins/manifest)。
...
4. 捆绑扩展（随 LCX Agent 一起发布，**默认禁用**）

- `<openclaw>/extensions/*`

捆绑插件必须通过 `plugins.entries.<id>.enabled` 或 `lcx plugins enable <id>` 显式启用。已安装的插件默认启用，但可以用相同方式禁用。
...
LCX Agent 还可以合并**外部渠道目录**（例如，MPM 注册表导出）。将 JSON 文件放在以下位置之一：
...
如果插件导出 `id`，LCX Agent 会使用它，但当它与配置的 id 不匹配时会发出警告。
...
LCX Agent 在运行时根据发现的插件增强 `uiHints`：
...
插件也可以注册自己的顶级命令（例如：`lcx voicecall`）。
...
```
import { registerPluginHooksFromDir } from "lcx-agent/plugin-sdk";
...
- 插件管理的钩子在 `lcx hooks list` 中显示为 `plugin:<id>`。
- 你不能通过 `lcx hooks` 启用/禁用插件管理的钩子；而是启用/禁用插件。

## 提供商插件（模型认证）

插件可以注册**模型提供商认证**流程，以便用户可以在 LCX Agent 内运行 OAuth 或 API 密钥设置（无需外部脚本）。
