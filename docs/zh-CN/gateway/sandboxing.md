...
status: active
summary: LCX Agent 沙箱隔离的工作原理：模式、作用域、工作区访问和镜像
title: 沙箱隔离
x-i18n:
...
LCX Agent 可以**在 Docker 容器内运行工具**以减少影响范围。
...
Skills 注意事项：`read` 工具以沙箱为根。使用 `workspaceAccess: "none"` 时，LCX Agent 将符合条件的 Skills 镜像到沙箱工作区（`.../skills`）以便可以读取。使用 `"rw"` 时，工作区 Skills 可从 `/workspace/skills` 读取。
...
默认镜像：`lcx-sandbox:bookworm-slim`
...
- 使用 `lcx sandbox explain` 检查生效的沙箱模式、工具策略和修复配置键。
