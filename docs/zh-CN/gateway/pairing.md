...
在 Gateway 网关拥有的配对中，**Gateway 网关**是允许哪些节点加入的唯一信息源。UI（Control UI、CLI、未来的客户端）只是审批或拒绝待处理请求的前端。
...
lcx nodes reject <requestId>
lcx nodes status
lcx nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
当满足以下条件时，客户端可以选择尝试**静默审批**：

- 请求标记为 `silent`，且
- 客户端可以使用相同用户验证到 Gateway 网关主机的 SSH 连接。
