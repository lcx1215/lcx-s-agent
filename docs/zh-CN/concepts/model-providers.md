...
- CLI 辅助工具：`lcx onboard`、`lcx models list`、`lcx models set <provider/model>`。

## 内置提供商（pi-ai 目录）

LCX Agent 附带 pi-ai 目录。这些提供商**不需要** `models.providers` 配置；只需设置认证 + 选择模型。
...
- CLI：`lcx onboard --auth-choice token`（粘贴 setup-token）或 `lcx models auth paste-token --provider anthropic`
...
- CLI：`lcx onboard --auth-choice openai-codex` 或 `lcx models auth login --provider openai-codex`
...
- Antigravity OAuth 作为捆绑插件提供（`google-antigravity-auth`，默认禁用）。
  - 启用：`lcx plugins enable google-antigravity-auth`
  - 登录：`lcx models auth login --provider google-antigravity --set-default`
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  - 启用：`lcx plugins enable google-gemini-cli-auth`
  - 登录：`lcx models auth login --provider google-gemini-cli --set-default`
...
lcx models set opencode/claude-opus-4-5
lcx models list
```
