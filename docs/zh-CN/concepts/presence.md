...
summary: LCX Agent 在线状态条目如何生成、合并和显示
title: 在线状态
x-i18n:
...
LCX Agent"在线状态"是以下内容的轻量级、尽力而为的视图：

- **Gateway 网关**本身，以及
- **连接到 Gateway 网关的客户端**（WebChat、Control UI、CLI 等）

在线状态为运维人员提供当前连接情况的快速可见性。
...
客户端可以通过 `system-event` 方法发送更丰富的周期性信标。客户端使用此方法报告主机名、IP 和 `lastInputSeconds`。
...
客户端渲染 `system-presence` 的输出，并根据最后更新的时间应用一个小的状态指示器（活跃/空闲/过期）。
