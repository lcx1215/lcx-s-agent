---
read_when:
  - 在 DigitalOcean 上设置 LCX Agent
  - 寻找便宜的 VPS 托管来运行 LCX Agent
summary: 在 DigitalOcean 上运行 LCX Agent（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 LCX Agent

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 LCX Agent Gateway 网关。
