---
summary: "How LCX Agent builds prompt context and reports token usage + costs"
read_when:
...
LCX Agent tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

LCX Agent assembles its own system prompt on every run. It includes:
...
For images, LCX Agent downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from LCX Agent session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `lcx status --usage` and `lcx channels list` show
...
`cacheWrite`. If pricing is missing, LCX Agent shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. LCX Agent can
...
Anthropic's 1M context window is currently beta-gated. LCX Agent can inject the
...
LCX Agent skips the `context-1m-*` beta header because Anthropic currently
