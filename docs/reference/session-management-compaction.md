...
This document explains how LCX Agent manages sessions end-to-end:
...
LCX Agent is designed around a single **Gateway process** that owns session state.
...
LCX Agent persists sessions in two layers:
...
LCX Agent resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, LCX Agent reports potential evictions but does not mutate the store/files.
...
LCX Agent intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (LCX Agent consumes the events, but Pi decides when to compact).
...
LCX Agent also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, LCX Agent bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, LCX Agent leaves it alone.
...
LCX Agent supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- LCX Agent strips/suppresses this in the delivery layer.

As of `2026.1.10`, LCX Agent also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
LCX Agent uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but LCX Agent’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `lcx status`.
