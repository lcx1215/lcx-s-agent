...
LCX Agent integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- LCX Agent owns lifecycle when `channels.signal.autoStart=true`.
...
- LCX Agent spawns `imsg rpc` as a child process (legacy iMessage integration).
