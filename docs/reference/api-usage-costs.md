...
LCX Agent features that can generate provider usage or paid API calls.
...
- `lcx status --usage` and `lcx channels list` show provider **usage windows**
...
LCX Agent can pick up credentials from:
...
`lcx models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
