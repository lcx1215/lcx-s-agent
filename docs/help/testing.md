...
LCX Agent has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Pool note:
  - LCX Agent uses Vitest `vmForks` on Node 22/23 for faster unit shards.
  - On Node 24+, LCX Agent automatically falls back to regular `forks` to avoid Node VM linking errors (`ERR_VM_MODULE_LINK_FAILURE` / `module is already linked`).
...
- Gemini API vs Gemini CLI:
  - API: LCX Agent calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: LCX Agent shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `lcx models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `lcx models list` / model selection.
