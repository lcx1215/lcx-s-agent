...
LCX Agent routes replies **back to the channel where a message came from**. The
...
LCX Agent infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, LCX Agent still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when LCX Agent would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
