...
Matrix is an open, decentralized messaging protocol. LCX Agent connects as a Matrix **user**
...
```bash
lcx plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   ...
   - From a local checkout: `lcx plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: LCX Agent calls the same
...
- On first connection, LCX Agent requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  LCX Agent logs a warning.
...
- Approve via:
  - `lcx pairing list matrix`
  - `lcx pairing approve matrix <CODE>`
...
- On startup, LCX Agent resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
