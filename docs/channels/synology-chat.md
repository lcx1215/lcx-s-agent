---
summary: "Synology Chat webhook setup and LCX Agent config"
read_when:
  - Setting up Synology Chat with LCX Agent
...
```bash
lcx plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your LCX Agent gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in LCX Agent.
...
- Pairing approvals work with:
  - `lcx pairing list synology-chat`
  - `lcx pairing approve synology-chat <CODE>`
...
```bash
lcx message send --channel synology-chat --target 123456 --text "Hello from LCX Agent"
lcx message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
