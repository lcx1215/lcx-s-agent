...
read_when:
  - You want LCX Agent to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables LCX Agent to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`lcx onboard`) and `lcx channels add` list optional channel plugins.
...
```bash
lcx plugins install --link <path-to-openclaw>/extensions/nostr
```
