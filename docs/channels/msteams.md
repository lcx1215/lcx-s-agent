...
```bash
lcx plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
LCX Agent will offer the local install path automatically.
...
3. Configure LCX Agent with those credentials.
...
- Talk to LCX Agent via Teams DMs, group chats, or channels.
...
- On startup, LCX Agent resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring LCX Agent, you need to create an Azure Bot resource.
...
1. **Install the Microsoft Teams plugin**
   ...
   - From a local checkout: `lcx plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "LCX Agent in Teams", "full": "LCX Agent in Teams" },
...
LCX Agent handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, LCX Agent only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
LCX Agent sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `lcx message poll --channel msteams --target conversation:<id> ...`
...
```bash
lcx message send --channel msteams \
...
lcx message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
lcx message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
lcx message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
lcx message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
