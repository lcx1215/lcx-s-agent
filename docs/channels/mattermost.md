---
summary: "Mattermost bot setup and LCX Agent config"
read_when:
...
```bash
lcx plugins install ./extensions/mattermost
```
...
4. Configure LCX Agent and start the gateway.
...
Native slash commands are opt-in. When enabled, LCX Agent registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, LCX Agent derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as LCX Agent.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to LCX Agent.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from LCX Agent, not `404`.
...
- Approve via:
  - `lcx pairing list mattermost`
  - `lcx pairing approve mattermost <CODE>`
...
Use these target formats with `lcx message send` or cron/webhooks:
