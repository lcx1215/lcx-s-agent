...
Tlon is a decentralized messenger built on Urbit. LCX Agent connects to your Urbit ship and can
...
```bash
lcx plugins install ./extensions/tlon
```
...
By default, LCX Agent blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `lcx message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, LCX Agent replies in-thread.
