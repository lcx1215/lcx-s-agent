...
read_when:
  - Setting up Zalo Personal for LCX Agent
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside LCX Agent.
...
- Or from a source checkout: `lcx plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `lcx channels login --channel zalouser`
...
```bash
lcx directory self --channel zalouser
lcx directory peers list --channel zalouser --query "name"
lcx directory groups list --channel zalouser --query "work"
```
...
- `lcx pairing list zalouser`
- `lcx pairing approve zalouser <code>`
...
- On startup, LCX Agent resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
Accounts map to `zalouser` profiles in LCX Agent state. Example:
...
- LCX Agent sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, LCX Agent sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `lcx channels logout --channel zalouser && lcx channels login --channel zalouser`
...
- The channel now runs fully in LCX Agent without external CLI binaries.
