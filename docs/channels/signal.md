...
- LCX Agent installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "LCX Agent"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure LCX Agent and restart the gateway.
5. Send a first DM and approve pairing (`lcx pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "LCX Agent"` then scan the QR in Signal.
...
4. Configure LCX Agent, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `lcx pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point LCX Agent at it:
...
This skips auto-spawn and the startup wait inside LCX Agent. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `lcx pairing list signal`
  - `lcx pairing approve signal <CODE>`
...
- **Typing indicators**: LCX Agent sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, LCX Agent forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `lcx doctor --fix`.
