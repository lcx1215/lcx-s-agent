...
read_when:
  - You need detailed behavior for lcx onboard
...
This page is the full reference for `lcx onboard`.
...
<Steps>
  <Step title="Existing config detection">
    ...
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `lcx doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `lcx pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `lcx health`.
    - `lcx status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
`lcx agents add` writes `agents.list[]` and optional `bindings`.
...
Clients (for example the Control UI) can render steps without re-implementing onboarding logic.
...
- Command reference: [`lcx onboard`](/cli/onboard)
