...
The onboarding wizard is the **recommended** way to set up LCX Agent on macOS,
...
`lcx dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
(`web_fetch` works without a key). Easiest path: `lcx configure --section web`
...
If the config is invalid or contains legacy keys, the wizard asks you to run `lcx doctor` first.
</Note>
...
Use `lcx agents add <name>` to create a separate agent with its own workspace,
...
- CLI command reference: [`lcx onboard`](/cli/onboard)
