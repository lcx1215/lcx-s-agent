...
This guide summarizes a sane workflow for working on the pi integration in LCX Agent.
...
- Trigger the agent directly:
  - `pnpm lcx agent --message "Hello" --thinking low`
...
State lives under the LCX Agent state directory. Default is `~/.openclaw`. If `OPENCLAW_STATE_DIR` is set, use that directory instead.
