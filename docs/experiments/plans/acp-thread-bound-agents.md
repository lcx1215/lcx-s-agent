...
This plan defines how LCX Agent should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- LCX Agent core owns ACP control plane concerns
...
LCX Agent should not reimplement ACP transport internals in core.
LCX Agent should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in LCX Agent, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under LCX Agent state dir
...
Core (must be in LCX Agent core):
...
- ACP event stream is normalized to LCX Agent reply chunks
...
For the first production backend (`extensions/acpx`), LCX Agent and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses LCX Agent `sessionKey`)
...
- LCX Agent consumes ndjson events from `acpx --format json --json-strict`
