...
are only safe when the Gateway is stopped. Prefer `lcx cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, LCX Agent applies a
...
If `delivery` is omitted for isolated jobs, LCX Agent defaults to `announce`.
...
When a job fails, LCX Agent classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, LCX Agent archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `lcx cron runs` and adjust retention before logs become large
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`lcx system event`](/cli/system).
...
- LCX Agent applies exponential retry backoff for recurring jobs after consecutive errors:
