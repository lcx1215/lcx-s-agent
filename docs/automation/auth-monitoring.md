...
LCX Agent exposes OAuth expiry health via `lcx models status`. Use that for
...
- `scripts/claude-auth-status.sh` now uses `lcx models status --json` as the
...
- `scripts/claude-auth-status.sh`: Claude Code + LCX Agent auth checker (full/json/simple).
...
- `scripts/termux-sync-widget.sh`: sync Claude Code creds → LCX Agent.
