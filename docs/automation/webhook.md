...
read_when:
  ...
  - Wiring external systems into LCX Agent
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your LCX Agent config directory (typically `~/.openclaw/hooks/transforms`).
...
- `lcx webhooks gmail setup` writes `hooks.gmail` config for `lcx webhooks gmail run`.
