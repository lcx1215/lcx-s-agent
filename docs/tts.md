...
LCX Agent can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere LCX Agent can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, LCX Agent defaults
...
- If `provider` is **unset**, LCX Agent prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, LCX Agent retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, LCX Agent:
...
Discord note: `/tts` is a built-in Discord command, so LCX Agent registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from LCX Agent
```
