...
```bash
lcx tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Check the Gateway logs: `lcx logs --follow`.
- Confirm the agent can run: `lcx status` and `lcx models status`.
...
- No agents in picker: check `lcx agents list` and your routing config.
