...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in LCX Agent
...
Running LCX Agent via Node with `tsx` fails at startup with:
...
- Before that (Bun path), `lcx status` and `gateway:watch` worked.
