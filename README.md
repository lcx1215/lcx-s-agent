...
6. 允许为了 L5 能力提升主动新增或重塑模块、CLI、评测、receipt、prompt、workflow glue 和文档入口。
7. 大脑能力必须单调：难题会做，简单前置题必须更会做。复杂评测、训练 promotion 和学习 receipt 不能绕过对应的简单用户入口。
8. 仍然禁止无关 provider、交易执行层、假 live-fixed、保护记忆覆盖，以及没有验证指标的炫技式扩张。
