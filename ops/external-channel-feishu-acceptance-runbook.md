...
- it describes the current `lcx-s-openclaw` -> `~/.openclaw/external-channel-runtime/lcx-s-openclaw` path
...
In `~/.openclaw/external-channel-runtime/lcx-s-openclaw`, confirm the equivalent seam exists before porting.
