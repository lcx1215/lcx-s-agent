...
4. [completed locally] Extend the finance data gateway with a preferred/fallback
   adapter-health receipt, using fixtures first and no credentials. The live
   public Yahoo adapter remains delayed and a single primary source, so the
   canonical gateway still blocks until cross-check evidence exists.
