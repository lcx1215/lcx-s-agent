...
| [EchoAPI](https://www.echoapi.com/wiki/docs/start)                                        | API test/client project       | HTTP, SSE, and TCP request client; the CLI runs one or more saved interface/test cases and exports JSON reports                | real public-case execution, response-contract checks, timing/status receipt, and local CI integration                              | CLI is optional because its npm package is `UNLICENSED`; no webhooks, client certs, secret env, or finance authority |
...
1. **Finance truth layer:** source registry, field definition, timestamp,
   ...
   Coinbase, CoinCap). Alpha Vantage and CoinGecko are available as explicitly
   keyed cross-check adapters; no key is stored in the repository.
   Geospatial data has its own registry for Open-Meteo, Nominatim, and USGS so
...
- `EchoAPI` is a request/test harness, not a market-data authority. LCX now has
  a real optional CLI runner for one public case iteration plus a JSON/status
  receipt. It sends no webhook, does not inherit secret environment variables,
  and does not grant finance or trading authority. The CLI is not a hard
  dependency because the published npm package is marked `UNLICENSED`.
- Public API adapters are live-capable, not fixture-only: crypto exchange data
  goes through Binance, Kraken, Coinbase, and CoinCap; geospatial context goes
  through Open-Meteo, Nominatim, and USGS. Each source can fail independently
  and remains visible in the receipt.
