...
- Matrix credentials: the explicit credentials read/write/remove adapter in
  `extensions/matrix/src/matrix/credentials.ts`; Matrix crypto/storage remains
  a separate directory-level transaction.
...
| Channel-local state     | Telegram/Discord/Web stores plus `src/plugins/services.ts`, `extensions/nostr/src/nostr-state-store.ts`, `extensions/matrix/src/matrix/credentials.ts`   | Nostr, Matrix credentials, Telegram offset, and Discord thread-binding adapters; sticker cache, Matrix crypto/storage, OAuth and plugin service state remain | Adapter-specific path injection; this does not prove external-channel binding or visible delivery        |
