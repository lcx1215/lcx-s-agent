...
- device identity and node-host state: the explicit read/write/rollback
  adapters in `src/infra/device-identity.ts` and `src/node-host/config.ts`,
  including key validation and secret-free audit records;
...
- channel pairing requests and allowlists: the explicit file adapter in
  ...
  `allowFrom` files.
- Zalo credentials: the explicit credential adapter in
  `extensions/zalouser/src/zalo-js.ts`; credential values remain outside audit
  records and the runtime keeps the canonical default path.
- local device settings and pairing notifications: the explicit adapters in
  `src/infra/voicewake.ts` and `extensions/device-pair/notify.ts`; these retain
  the same backup, audit, expected-path, and rollback contract without binding
  a transport.
...
| Channel-local state     | Telegram/Discord/Web stores, channel pairing, MSTeams, external replay, voice-call, plus `src/plugins/services.ts`, `extensions/nostr/src/nostr-state-store.ts`, `extensions/matrix/src/matrix/credentials.ts`, `extensions/matrix/src/matrix/client/storage.ts`, `extensions/zalouser/src/zalo-js.ts` | Nostr, Matrix credentials/storage, Zalo credentials, Telegram offset/sticker/command-hash cache, Discord binding/model-picker, channel pairing, WhatsApp Web auth, MSTeams, external replay, and voice-call adapters; custom/provider-owned state remains | Adapter-specific path injection; this does not prove external-channel binding or visible delivery       |
...
The following direct writers are also intentionally outside identity
migration: `src/infra/push-apns.ts` stores external push registrations,
`src/infra/gateway-lock.ts` stores a process lock, and raw-stream,
command-logger, browser, Canvas, completion, media, and temporary files are
operational artifacts rather than user identity state. They keep their existing
owners and are not evidence of external-channel binding or visible delivery.
