...
### 已加的第二个源：宏观（2026-09-21）

新增 `src/agents/finance-macro-signal.ts` 并接进采样器。它读 FRED 官方序列，**与标的自身的价格
数据无关** —— 这是它能算"独立来源"的唯一理由。每个标的显式声明序列、传导依据与强度上限：

| 标的            | 序列                      | 传导依据                                       | 置信上限 |
| --------------- | ------------------------- | ---------------------------------------------- | -------- |
| TLT             | `DGS10` 10y 收益率        | 债券价格与收益率反向 —— **恒等式**             | 0.6      |
| GLD             | `DFII10` 10y 实际利率     | 黄金无息，实际利率即持有成本                   | 0.5      |
| DBC             | `DCOILWTICO` WTI 原油     | 能源是商品篮子的最大权重                       | 0.45     |
| SPY / QQQ / IWM | `BAMLH0A0HYM2` 高收益利差 | 信用市场在给压力定价 —— **代理，非估值**       | 0.45     |
| EFA / EEM       | `DTWEXBGS` 美元指数       | 美元走强压低非美资产的美元价值 —— **换算效应** | 0.45     |

上限不一般高：恒等式高于代理关系，这是写进代码的、可以被反驳的判断，而不是埋在常量里。

**实测（真实 FRED 数据，8 个标的全部发出方向）**：

```
TLT sell str=1 conf=0.6      GLD sell str=1 conf=0.5     DBC buy str=1 conf=0.45
...
要真正闭合，需要**第二个能在 ETF 上表达方向的独立源**（期权 put/call、ETF 资金流、新闻情绪）。
⚠️ 绝对不要为了凑够 2 个源把 `minSources` 降到 1 —— 那正是融合层存在的目的。
