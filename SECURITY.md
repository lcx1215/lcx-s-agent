...
If you believe you've found a security issue in LCX Agent, please report it privately.

## Reporting

Report vulnerabilities privately in this repository:

- **Private report (preferred)** — [Report a vulnerability](https://github.com/lcx1215/lcx-s-agent/security/advisories/new)
- **Public discussion** — [GitHub discussions](https://github.com/lcx1215/lcx-s-agent/discussions), only after a fix is released or the issue is already public

This project is independently maintained. It is not affiliated with, and does not forward reports
to, any third-party project — please do not send reports about it anywhere else.

For general security guidance see [Gateway security](https://docs.openclaw.ai/gateway/security).
...
- Tested version details (LCX Agent version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to LCX Agent's documented trust boundaries.
- For exposed-secret reports: proof the credential is LCX Agent-owned (or grants access to LCX Agent-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at LCX Agent. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

LCX Agent is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
LCX Agent does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- LCX Agent can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need LCX Agent, use one VPS (or host/OS user boundary) per user.
...
- This is expected in LCX Agent's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of LCX Agent's trusted computing base for a gateway.
...
- Using LCX Agent in ways that the docs recommend not to
...
- Exposed secrets that are third-party/user-controlled credentials (not LCX Agent-owned and not granting access to LCX Agent-operated infrastructure/services) without demonstrated LCX Agent impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

LCX Agent security guidance assumes:

- The host where LCX Agent runs is within a trusted OS/admin boundary.
...
LCX Agent's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
LCX Agent separates routing from execution, but both remain inside the same operator trust boundary:
...
- Plugins can execute with the same OS privileges as the LCX Agent process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

LCX Agent uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:
...
- Sandbox media validation allows absolute temp paths only under the LCX Agent-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use LCX Agent temp helpers (`resolvePreferredOpenClawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
...
For threat model + hardening guidance (including `lcx security audit --deep` and `--fix`), see:
...
LCX Agent's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `lcx gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - LCX Agent keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `lcx security audit` as dangerous findings.
...
LCX Agent requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running LCX Agent in Docker:
...
```bash
docker build -t lcx-agent:local .
docker run --read-only --cap-drop=ALL \
  -v lcx-agent-data:/app/data \
  lcx-agent:local
```
