...
async function runTargetReports(params: {
  manifest: FundamentalManifestScaffold;
  documents?: Array<{ fileName: string; metadata?: FundamentalDocumentMetadata }>;
}): Promise<{
  ...
  const reportsDir = path.join(tempDir, "bank", "fundamental", "target-reports");
  ...
  try {
    ...
    artifact = JSON.parse(await fs.readFile(path.join(reportsDir, files[0]), "utf-8")) as Record<
      string,
      unknown
    >;
  } catch {}
  ...
  try {
    ...
    noteContent = await fs.readFile(
      path.join(
        memoryDir,
        memoryFiles.find((name) => name.includes("fundamental-target-reports-")) ?? memoryFiles[0],
      ),
      "utf-8",
    );
  } catch {}
  ...
  return { artifact, noteContent, report };
}

beforeAll(async () => {
  ({ default: handler } = await import("./handler.js"));
  suiteWorkspaceRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "openclaw-fundamental-target-reports-"),
  );
});
...
describe("fundamental-target-reports hook", () => {
  it("skips blocked targets with no dossier-ready packets", async () => {
    const manifest = createManifestFixture();
    const result = await runTargetReports({ manifest });
  ...
  it("skips collection-only targets with no dossier-ready packets", async () => {
    ...
    const result = await runTargetReports({
      manifest,
      documents: [
  ...
  it("writes target reports for dossier-ready targets", async () => {
    const manifest = createManifestFixture({
      reviewGateStatus: "approved_for_collection",
      requestText:
    ...
    expect(result.artifact).toMatchObject({
      ...
      reportFiles: [
        expect.objectContaining({
          targetLabel: "AAPL",
          relativePath: `bank/fundamental/reports/${manifest.manifestId}/aapl.md`,
        }),
      ],
    });
    expect(result.noteContent).toContain("report_files: 1");
    expect(result.report).toContain("# AAPL Fundamental Research Report");
