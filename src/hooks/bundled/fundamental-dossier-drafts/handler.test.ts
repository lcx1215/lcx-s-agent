...
async function runDossierDrafts(params: {
  manifest: FundamentalManifestScaffold;
  documents?: Array<{ fileName: string; metadata?: FundamentalDocumentMetadata }>;
}): Promise<{
  ...
  const draftsDir = path.join(tempDir, "bank", "fundamental", "dossier-drafts");
  ...
  try {
    ...
    artifact = JSON.parse(
      await fs.readFile(path.join(draftsDir, draftFiles[0]), "utf-8"),
  ...
  try {
    ...
    noteContent = await fs.readFile(
      path.join(
        memoryDir,
        memoryFiles.find((name) => name.includes("fundamental-dossier-drafts-")) ?? memoryFiles[0],
      ),
      "utf-8",
    );
  } catch {}
  ...
  return { artifact, noteContent, draft };
}

beforeAll(async () => {
  ({ default: handler } = await import("./handler.js"));
  suiteWorkspaceRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "openclaw-fundamental-dossier-drafts-"),
  );
});
...
describe("fundamental-dossier-drafts hook", () => {
  it("skips blocked targets with no dossier-ready packets", async () => {
    const manifest = createManifestFixture();
    const result = await runDossierDrafts({ manifest });
  ...
  it("skips collection-only targets with no dossier-ready packets", async () => {
    const manifest = createManifestFixture({
      reviewGateStatus: "approved_for_collection",
    });
    const result = await runDossierDrafts({
      manifest,
      documents: [
  ...
  it("writes dossier drafts for dossier-ready targets", async () => {
    const manifest = createManifestFixture({
      reviewGateStatus: "approved_for_collection",
      requestText:
    ...
    expect(result.artifact).toMatchObject({
      ...
      draftFiles: [
        expect.objectContaining({
          targetLabel: "AAPL",
          relativePath: `bank/fundamental/drafts/${manifest.manifestId}/aapl.md`,
        }),
      ],
    });
    expect(result.noteContent).toContain("draft_files: 1");
    expect(result.draft).toContain("# AAPL Fundamental Dossier Draft");
    expect(result.draft).toContain("## Executive Summary Draft");
    expect(result.draft).toContain("## Citation Plan");
  });
});
