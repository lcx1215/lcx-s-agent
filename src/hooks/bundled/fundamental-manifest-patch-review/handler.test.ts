...
async function runPatchReview(params: {
  manifest: FundamentalManifestScaffold;
  documents?: Array<{ fileName: string; metadata?: FundamentalDocumentMetadata }>;
}): Promise<{ artifact: Record<string, unknown> | null; noteContent: string | null }> {
  ...
  const reviewsDir = path.join(tempDir, "bank", "fundamental", "manifest-patch-reviews");
  ...
  try {
    ...
    artifact = JSON.parse(await fs.readFile(path.join(reviewsDir, files[0]), "utf-8")) as Record<
      string,
      unknown
    >;
  } catch {}
  ...
  try {
    ...
    noteContent = await fs.readFile(
      path.join(
        memoryDir,
        memoryFiles.find((name) => name.includes("fundamental-manifest-patch-review-")) ??
          memoryFiles[0],
      ),
      "utf-8",
    );
  ...
  return { artifact, noteContent };
}

beforeAll(async () => {
  ({ default: handler } = await import("./handler.js"));
  suiteWorkspaceRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "openclaw-fundamental-manifest-patch-review-"),
  );
});
...
describe("fundamental-manifest-patch-review hook", () => {
  it("skips blocked targets with no patch proposals", async () => {
    const manifest = createManifestFixture();
    const result = await runPatchReview({ manifest });

    expect(result.artifact).toBeNull();
    expect(result.noteContent).toBeNull();
  });

  it("writes review artifacts for collection targets", async () => {
    const manifest = createManifestFixture({
      reviewGateStatus: "approved_for_collection",
    });
    const result = await runPatchReview({
      manifest,
      documents: [
    ...
    expect(result.noteContent).toContain("review_status: review_ready");
  });

  it("skips dossier-ready targets with no patch proposals", async () => {
    const manifest = createManifestFixture({
      reviewGateStatus: "approved_for_collection",
      requestText:
