...
async function runCollectionPackets(params: {
  manifest: FundamentalManifestScaffold;
  documents?: Array<{ fileName: string; metadata?: FundamentalDocumentMetadata }>;
}): Promise<{
  ...
  try {
    packetContent = await fs.readFile(packetWorkfilePath, "utf-8");
  } catch {}

  return { artifact, noteContent, packetContent };
}

beforeAll(async () => {
  ({ default: handler } = await import("./handler.js"));
  suiteWorkspaceRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "openclaw-fundamental-collection-packets-"),
  );
});
...
describe("fundamental-collection-packets hook", () => {
  it("skips blocked targets with no collection patch proposals", async () => {
    const manifest = createManifestFixture();
    const result = await runCollectionPackets({ manifest });

    expect(result.artifact).toBeNull();
    expect(result.noteContent).toBeNull();
    expect(result.packetContent).toBeNull();
  });

  it("writes collection packets for follow-up collection targets", async () => {
    const manifest = createManifestFixture({
      reviewGateStatus: "approved_for_collection",
    });
    const result = await runCollectionPackets({
      manifest,
      documents: [
    ...
    expect(result.noteContent).toContain("status: pending_collection");
    expect(result.packetContent).toContain("# AAPL Collection Patch Packet");
    expect(result.packetContent).toContain("## Manual Checks");
  });

  it("skips dossier-ready targets with no collection patch proposals", async () => {
    const manifest = createManifestFixture({
      reviewGateStatus: "approved_for_collection",
      requestText:
