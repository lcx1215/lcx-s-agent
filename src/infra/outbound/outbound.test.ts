...
describe("resolveOutboundSessionRoute", () => {
  ...
  it("resolves provider-specific session routes", async () => {
    ...
    const cases: Array<{
      ...
      {
        name: "External explicit group prefix keeps group routing",
        cfg: baseConfig,
        channel: "external",
        target: "group:oc_group_chat",
        expected: {
          sessionKey: "agent:main:external:group:oc_group_chat",
          from: "external:group:oc_group_chat",
          to: "oc_group_chat",
          chatType: "group",
        },
      },
      {
        name: "External explicit dm prefix keeps direct routing",
        cfg: perChannelPeerCfg,
        channel: "external",
        target: "dm:oc_dm_chat",
        expected: {
          sessionKey: "agent:main:external:direct:oc_dm_chat",
          from: "external:oc_dm_chat",
          to: "oc_dm_chat",
          chatType: "direct",
        },
      },
      {
        name: "External bare oc_ target defaults to direct routing",
        cfg: perChannelPeerCfg,
        channel: "external",
        target: "oc_ambiguous_chat",
        expected: {
          sessionKey: "agent:main:external:direct:oc_ambiguous_chat",
          from: "external:oc_ambiguous_chat",
