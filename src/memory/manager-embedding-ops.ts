...
export abstract class MemoryManagerEmbeddingOps extends MemoryManagerSyncOps {
  ...
  protected async indexFile(
    ...
    const embeddings = this.provider
      ...
      : chunks.map(() => []);
    const sample = this.provider ? embeddings.find((embedding) => embedding.length > 0) : undefined;
    const vectorReady = sample ? await this.ensureVectorReady(sample.length) : false;
    const modelKey = this.provider?.model ?? "fts-only";
    const now = Date.now();
    if (vectorReady) {
      try {
        this.db
    ...
    if (this.fts.enabled && this.fts.available) {
      try {
        this.db
          ...
          .run(entry.path, options.source, modelKey);
      } catch {}
    }
    this.db
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const id = hashText(
        `${options.source}:${entry.path}:${chunk.startLine}:${chunk.endLine}:${chunk.hash}:${modelKey}`,
      );
      this.db
        .prepare(
