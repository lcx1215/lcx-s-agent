...
function resolveLearningCouncilModel(role: LearningCouncilRole, cfg: OpenClawConfig): string {
  ...
  switch (role) {
    ...
    case "deepseek":
      return (
        // The extraction lane is latency-sensitive inside the External reply loop.
...
function buildDeepSeekSystemPrompt(params?: {
  ...
  return [
    ...
    "Be stricter than the synthesis lane: extract claims, missing proof, source timestamp gaps, schema violations, and boundary leaks instead of smoothing the topic into a nicer answer.",
    "Never upgrade application_ready, source receipts, or successful retrieval into eval_absorbed unless Qwen eval or training absorption evidence is explicitly present.",
    "For finance or market claims, flag missing source timestamp, field definition, unit/currency, adjusted status, provider role, and data-gateway proof.",
    "For research-only answers, flag trade-like wording such as buy, sell, hold, add, reduce, wait, 关注, 值得, or better entry.",
...
export async function runExternalLearningCouncil(params: {
  cfg: OpenClawConfig;
  userMessage: string;
  routeAgentId: string;
  sessionKey: string;
