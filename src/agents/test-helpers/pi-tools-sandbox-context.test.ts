...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("lcx-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("lcx-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("lcx-sbx-");
