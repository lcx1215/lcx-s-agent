...
import { canonicalizeLcxOntologyValue } from "../shared/lcx-ontology.js";
import {
  normalizeExternalApiReplyForDistillation,
  type ExternalApiReplyDistillationSample,
} from "./external-api-reply-distillation.js";

export type ExternalBrainDistillationCandidateSource =
  | "api_reply"
  | "external_visible_reply"
  | "external_user_utterance"
  | "teacher_review";

export type ExternalBrainDistillationCandidateStatus =
  | "pending_brain_review"
  | "accepted_brain_plan"
  | "rejected_brain_plan"
  | "discarded";

export type ExternalBrainDistillationCandidate = {
  id: string;
  source: ExternalBrainDistillationCandidateSource;
  status: ExternalBrainDistillationCandidateStatus;
  boundary: "brain_distillation_candidate";
  createdAt: string;
  sample: ExternalApiReplyDistillationSample;
  userMessage?: string;
...
export type ExternalBrainDistillationCandidateArtifact = {
  schemaVersion: 1;
  boundary: "brain_distillation_candidate";
  generatedAt: string;
  ...
  candidates: ExternalBrainDistillationCandidate[];
};

export type ExternalBrainDistillationReviewArtifact = {
  schemaVersion: 1;
  ...
  acceptedCandidates: ExternalBrainDistillationCandidate[];
...
export function buildExternalBrainDistillationCandidate(params: {
  source: ExternalBrainDistillationCandidateSource;
  payload: unknown;
  userMessage?: string;
  createdAt?: string;
  review?: ExternalBrainDistillationCandidate["review"];
}): ExternalBrainDistillationCandidate {
  const sample = normalizeExternalApiReplyForDistillation(params.payload);
  ...
  const status: ExternalBrainDistillationCandidateStatus =
...
export function buildExternalBrainDistillationCandidateArtifact(params: {
  candidates: ExternalBrainDistillationCandidate[];
  generatedAt?: string;
}): ExternalBrainDistillationCandidateArtifact {
  return {
    schemaVersion: 1,
    boundary: "brain_distillation_candidate",
