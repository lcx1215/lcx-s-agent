...
describe("doctor state integrity oauth dir checks", () => {
  ...
  it("prints openclaw-only verification hints when recent sessions are missing transcripts", async () => {
    ...
    expect(text).toMatch(/lcx sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/lcx sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(/lcx sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/);
