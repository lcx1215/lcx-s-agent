...
const AUTO_CUE_RULES: SkillAutoCueRule[] = [
  ...
  },
  {
    ...
    patterns: [
      ...
      /WildClawBench/i,
      /Agent S/i,
      /黑科技.{0,16}(融入|接入|升级|吸收|容纳)/,
      /SkillOpt.{0,16}(v2|升级|生命周期|融入)/i,
      /(trajectory|轨迹).{0,16}(schema|训练|trace|融入|接入)/i,
      /(trace|observability|观测).{0,16}(agent|智能体|side-effect|副作用|融入)/i,
      /(安全|permission|权限).{0,16}(tool|skill|MCP|工具|技能)/i,
