...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the LCX Agent gateway (\`${formatCliCommand("openclaw gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the LCX Agent browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the LCX Agent browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
