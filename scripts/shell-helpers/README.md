...
Inspired by Simon Willison's [Running LCX Agent in Docker](https://til.simonwillison.net/llms/openclaw-docker).
...
On first command, ClawDock auto-detects your LCX Agent directory:
...
| `clawdock-cli <command>`  | Run LCX Agent CLI commands                     |
...
| `clawdock-cd`        | Jump to the LCX Agent project directory   |
| `clawdock-config`    | Open the LCX Agent config directory       |
...
```bash
lcx channels login --channel whatsapp --verbose
```
...
2. Configure it in the LCX Agent config
...
- Bash or Zsh shell
- LCX Agent project (from `docker-setup.sh`)
