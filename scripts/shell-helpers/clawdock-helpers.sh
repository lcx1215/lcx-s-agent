#!/usr/bin/env bash
# ClawDock - Docker helpers for LCX Agent
# Inspired by Simon Willison's "Running LCX Agent in Docker"
...
# Common paths to check for LCX Agent
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found LCX Agent at: $found_path"
    ...
    echo ""
    echo "❌ LCX Agent not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/lcx1215/lcx-s-agent.git ~/openclaw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d lcx-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart lcx-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f lcx-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec lcx-gateway \
...
clawdock-cli() {
  _clawdock_compose run --rm lcx-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build lcx-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENCLAW_GATEWAY_TOKEN=$token" lcx-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" lcx-gateway \
  ...
  local saved_token
  saved_token=$(_clawdock_compose exec lcx-gateway \
  ...
  _clawdock_compose restart lcx-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm lcx-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec lcx-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'lcx config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (lcx alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to lcx project directory${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'lcx channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'lcx status')"
  echo ""
