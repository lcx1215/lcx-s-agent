...
function usage(): never {
  throw new Error(
    [
      "Usage: node --import tsx scripts/operator/lcx-external-short-intent-fuzzer.ts [--json] [--max-per-family N]",
      "",
      "Generates external-channel short-intent variants by failure family and runs them through the commercial answer pipeline.",
...
const FUZZ_FAMILIES: FuzzFamily[] = [
  ...
  },
  {
    ...
    capabilityFamily: "external_short_intake",
...
export function runExternalShortIntentFuzzer(options: { maxPerFamily?: number } = {}) {
  ...
  return {
    ok: failed.length === 0,
    boundary: "local_external_message_short_intent_fuzzer_only",
    macroContract: {
      notWhitelist: true,
      strategy:
...
function renderText(payload: ReturnType<typeof runExternalShortIntentFuzzer>): string {
  return [
    `ok=${payload.ok}`,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const payload = runExternalShortIntentFuzzer({ maxPerFamily: options.maxPerFamily });
