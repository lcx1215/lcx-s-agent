...
function usage(): never {
  throw new Error(
    [
      "Usage: node --import tsx scripts/operator/local-brain-open-eval.ts [--json] [--provider-command CMD] [--provider-timeout-ms N]",
      "",
...
function parseArgs(args: string[]): CliOptions {
  const options: CliOptions = {
    providerCommand: "node --import tsx scripts/operator/local-brain-open-eval-provider.ts",
    json: false,
