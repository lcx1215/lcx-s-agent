...
const STATE_FILES = [
  "branches/_system/host_watchdog_state.json",
...
function parseArgs(argv: string[]): Args {
  ...
  return {
    ...
    plistPath: path.resolve(readValue("--plist") ?? DEFAULT_HOST_WATCHDOG_PLIST),
...
export function buildHostWatchdogDryRunReport(params: {
  legacyRoot: string;
  targetRoot: string;
  plistPath: string;
  checkedAt?: string;
}): HostWatchdogDryRunReport {
  const legacyRequiredFiles = checkFiles(params.legacyRoot, REQUIRED_HOST_WATCHDOG_FILES);
  const targetRequiredFiles = checkFiles(params.targetRoot, REQUIRED_HOST_WATCHDOG_FILES);
  ...
  if (!launchAgent.exists) {
    ...
    blockedReasons.push("host watchdog LaunchAgent no longer points at the audited legacy root");
  }
  if (missingLegacy.length > 0) {
    blockedReasons.push(
      `legacy host watchdog dependency missing: ${missingLegacy.map((entry) => entry.relativePath).join(", ")}`,
    );
  }
  if (missingTarget.length > 0) {
    blockedReasons.push(
      `target clean repo still lacks host watchdog dependency: ${missingTarget.map((entry) => entry.relativePath).join(", ")}`,
    );
  }
  if (untrackedTarget.length > 0) {
    blockedReasons.push(
      `target host watchdog dependency exists but is not tracked by Git: ${untrackedTarget.map((entry) => entry.relativePath).join(", ")}`,
    );
  }
...
function renderText(report: HostWatchdogDryRunReport): string {
  const lines = [
    `hostWatchdogDryRun=${report.migrationReady ? "migration_ready" : "blocked"}`,
    `mode=${report.mode}`,
...
export function main(argv = process.argv.slice(2)): number {
  ...
  const report = buildHostWatchdogDryRunReport(args);
