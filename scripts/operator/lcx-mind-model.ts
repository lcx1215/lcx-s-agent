...
const MIND_MODEL_LANES: MindModelLane[] = [
  ...
  {
    id: "central_agent_decision_layer",
    masterLane: "agent_workflow_memory",
    objective:
      "Make the central agent the single gate-and-record decision layer over the governance owners, so a whole-system decision has one owner, one gate, and one receipt instead of an implicit side effect.",
    headTerms: ["Central Agent Harness", "lcx-central-agent-latest.json", "gate_and_record_only"],
    workflowTerms: [
      "scripts/operator/lcx-central-agent.ts",
      "src/agents/central-harness/tool-registry.ts",
      "centralAgent",
      ...
      "read_only_owners_plus_planning_only_capabilities",
    ],
    proofTerms: [
      "test/lcx-central-agent.test.ts",
      "lcx_central_agent_v1",
      "lcx_central_agent_latest_v1",
    ...
    nextAction:
      "Read the central-agent latest snapshot and receipt log before changing an owner lane; the brain proposes, the TypeScript gate approves or blocks, and the receipt is the only proof.",
  },
  {
...
function usage(): never {
  throw new Error(
    [
      "Usage: node --import tsx scripts/operator/lcx-mind-model.ts [--json]",
      "",
