...
function usage(): never {
  throw new Error(
    [
      "Usage: node --import tsx scripts/operator/lcx-doctrine-consistency.ts [--json]",
      "",
...
async function main() {
  ...
  const activeCode = [
    ["scripts/operator/local-brain-distill-eval.ts", evalScript],
    ["scripts/operator/local-brain-plan.ts", planScript],
    ["scripts/operator/lcx-system-doctor.ts", systemDoctor],
  ] as const;
  ...
  checks.push({
    ...
    summary: "active LCX doctrine and operator entrypoints should not describe the system as L3/L4",
