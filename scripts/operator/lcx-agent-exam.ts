...
function usage(): never {
  throw new Error(
    [
      "Usage: node --import tsx scripts/operator/lcx-agent-exam.ts [--json] [--live] [--l5]",
      "",
...
function buildLiveBoundaryLane(live: boolean, channelCommand: CommandResult | undefined): ExamLane {
  ...
  return {
    ...
    nextAction:
      "优先用自然短句查真实入站、answer_audit 和 outbound_result；acceptance phrase 只是可选收据锚点，缺真实入站/出站不能叫 user-visible-observed。",
  };
}
  ...
  return [
    {
      ...
      nextAction:
        "按 core-ready -> external-channel-bound -> user-visible-observed 顺序做迁移、重启、probe、真实入站回复验收。",
    },
    {
    ...
    },
    {
      ...
      evidence: [
        ...
        "command=node --import tsx scripts/operator/lcx-commercial-acceptance-harness.ts --json",
...
async function readCognitiveIntegritySources(): Promise<CognitiveIntegritySources> {
  ...
  const [
    ...
    read("AGENTS.md"),
    read("scripts/operator/local-brain-distill-eval.ts"),
...
export async function runAgentExam(options: CliOptions): Promise<ExamReport> {
  ...
  const doctor = await runCommand({
    ...
    args: ["--import", "tsx", "scripts/operator/lcx-system-doctor.ts", "--json"],
    parseJson: true,
  ...
  const trainingPlan = await runCommand({
    ...
    command: process.execPath,
    args: ["--import", "tsx", "scripts/operator/local-brain-training-plan.ts", "--json"],
    parseJson: true,
    timeoutMs: options.timeoutMs,
  });
  const promotionAudit = await runCommand({
    ...
    command: process.execPath,
    args: ["--import", "tsx", "scripts/operator/local-brain-promotion-audit.ts", "--json"],
    parseJson: true,
    timeoutMs: options.timeoutMs,
  });
