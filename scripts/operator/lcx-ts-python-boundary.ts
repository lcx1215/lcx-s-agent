...
const PYTHON_POLICIES: Record<string, Omit<PythonFilePolicy, "path">> = {
  ...
  "daily_learning_runner.py": {
    ...
    targetTsOwner: "scripts/operator/lcx-governance-autopilot.ts",
  },
  "scripts/lobster_paths.py": {
    role: "wrap_with_ts_owner",
    plainRole: "包装",
    reason: "这是旧路径辅助，迁完前只能被 TS 路径主人包住使用。",
    targetTsOwner: "scripts/operator/lcx-local-paths.ts",
  },
...
function parseArgs(args: string[]) {
  ...
  for (const arg of args) {
    if (arg === "--json") {
      ...
      throw new Error(
        "Usage: node --import tsx scripts/operator/lcx-ts-python-boundary.ts [--json]",
      );
    } else {
      throw new Error(
        "Usage: node --import tsx scripts/operator/lcx-ts-python-boundary.ts [--json]",
