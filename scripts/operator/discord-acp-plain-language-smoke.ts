...
function usage(): string {
  return (
    "Usage: bun scripts/operator/discord-acp-plain-language-smoke.ts " +
    ...
    "1) LCX Agent spawned an ACP thread binding\n" +
    ...
    "  --openclaw-bin <path>        LCX Agent CLI binary for driver=openclaw (default: openclaw)\n" +
...
async function run(): Promise<SuccessResult | FailureResult> {
  ...
  try {
    if (args.driverMode === "token") {
      ...
      if (!sentMessageId) {
        throw new Error("lcx message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from LCX Agent.`,
        diagnostics: {
          bindingCandidates: [
            {
