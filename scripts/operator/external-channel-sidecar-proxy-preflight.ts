...
const REQUIRED_FILES = [
  "external_event_proxy.py",
  "run_external_proxy.sh",
  "send_external_reply.sh",
...
export type ExternalProxyPreflightReceipt = {
  schemaVersion: 1;
  generatedAt: string;
  legacyRoot: string;
  targetRoot: string;
...
function inspectCurrentLaunchAgent(legacyRoot: string, targetRoot: string) {
  let text = "";
  try {
    text = fs.readFileSync(EXTERNAL_PROXY_PLIST, "utf8");
  } catch {
...
function runSmoke(targetRoot: string, smokePort: number): ExternalProxyPreflightReceipt["smoke"] {
  const commandResults: CommandResult[] = [];
  commandResults.push(
    run(
      "python3",
      ["-m", "py_compile", "external_event_proxy.py", "scripts/learning_goal_registry.py"],
      targetRoot,
    ),
  );
  ...
  const child = spawnSync(
    ...
    [
      ...
      [
        ...
        `export OPENCLAW_BIN=${JSON.stringify(path.join(targetRoot, "send_external_reply.sh"))}`,
        "python3 external_event_proxy.py > /tmp/openclaw-external-proxy-smoke.out 2> /tmp/openclaw-external-proxy-smoke.err & pid=$!",
        "sleep 1",
...
export function buildExternalProxyPreflightReceipt(params: {
  legacyRoot: string;
  targetRoot: string;
  outputDir: string;
  writeRuntime?: boolean;
  ...
  return {
    ...
    smoke,
    boundary: [
      "Copies only existing live external message proxy dependencies into the non-Desktop runtime.",
      "Uses an alternate smoke port and does not replace the live 3011 proxy.",
      "Does not modify LaunchAgents.",
      "Does not send external message messages.",
    ],
  };
}

function writeReceipt(receipt: ExternalProxyPreflightReceipt): void {
...
function renderText(receipt: ExternalProxyPreflightReceipt): string {
  const lines = [
    `externalProxyPreflight=${receipt.readyForLiveInstall ? "ready" : "blocked"}`,
...
export function main(argv = process.argv.slice(2)): number {
  ...
  const receipt = buildExternalProxyPreflightReceipt({
