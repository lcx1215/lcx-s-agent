...
function usage(): never {
  throw new Error(
    [
      "Usage: node --import tsx scripts/operator/lcx-context-recovery-exam.ts [--json] [--handoff]",
      "",
...
async function currentChangeImpactSnapshot(): Promise<{
  ...
  try {
    const { stdout } = await execFileAsync(
      process.execPath,
      ["--import", "tsx", "scripts/operator/lcx-change-impact-plan.ts", "--json"],
      { cwd: repoRoot, env: process.env, maxBuffer: 20 * 1024 * 1024 },
    );
    return { ok: true, payload: JSON.parse(stdout) as Record<string, unknown> };
  } catch (error) {
...
async function currentModuleAbsorptionGateSnapshot(): Promise<{
  ...
  try {
    const { stdout } = await execFileAsync(
      process.execPath,
      ["--import", "tsx", "scripts/operator/lcx-module-learning-absorption-gate.ts", "--json"],
      { cwd: repoRoot, env: process.env, maxBuffer: 20 * 1024 * 1024 },
    );
    return { ok: true, payload: JSON.parse(stdout) as Record<string, unknown> };
  } catch (error) {
...
async function currentLearningSedimentationAuditSnapshot(): Promise<{
  ...
  try {
    const { stdout } = await execFileAsync(
      process.execPath,
      ["--import", "tsx", "scripts/operator/lcx-learning-sedimentation-audit.ts", "--json"],
      { cwd: repoRoot, env: process.env, maxBuffer: 20 * 1024 * 1024 },
    );
    return { ok: true, payload: JSON.parse(stdout) as Record<string, unknown> };
  } catch (error) {
...
async function currentExternalChannelStatusSnapshot(): Promise<{
  ...
  try {
    const { stdout } = await execFileAsync(
      process.execPath,
      ["--import", "tsx", "scripts/operator/lcx-external-channel-status.ts", "--json"],
      { cwd: repoRoot, env: process.env, maxBuffer: 20 * 1024 * 1024 },
    );
    return { ok: true, payload: JSON.parse(stdout) as Record<string, unknown> };
  } catch (error) {
...
async function currentSelfRepairHandsSnapshot(): Promise<{
  ...
  try {
    const { stdout } = await execFileAsync(
      process.execPath,
      ["--import", "tsx", "scripts/operator/lcx-self-repair-hands.ts", "--json"],
      { cwd: repoRoot, env: process.env, maxBuffer: 20 * 1024 * 1024 },
    );
    return { ok: true, payload: JSON.parse(stdout) as Record<string, unknown> };
  } catch (error) {
...
function buildNewWindowHandoffText(params: {
  ...
  const lines = [
    ...
    `legacyLiveMatchesCurrentCanonical=${scalarText(params.externalChannelStatus?.liveMatchesCurrentCanonical)}`,
...
async function flowGraphCheck(): Promise<RecoveryCheck> {
  try {
    const { stdout } = await execFileAsync(
      process.execPath,
      ["--import", "tsx", "scripts/operator/lcx-flow-graph.ts", "--json"],
      { cwd: repoRoot, env: process.env, maxBuffer: 20 * 1024 * 1024 },
    );
    const payload = JSON.parse(stdout) as Record<string, unknown>;
    const summary = payload.summary as Record<string, unknown> | undefined;
...
async function universeIndexCheck(): Promise<RecoveryCheck> {
  try {
    ...
    return {
      ...
      ok: payload.ok === true && payload.boundary === "local_universe_index_only",
      summary:
...
async function main() {
  ...
  const [
    ...
    readText(path.join(repoRoot, "scripts/operator/lcx-change-impact-plan.ts")),
  ...
  const operatorFlowMatchesCurrent =
    ...
    latestFlowGraph?.boundary === "local_flow_graph_only" &&
  ...
  const checks: RecoveryCheck[] = [
    ...
    universeIndex,
    {
      ...
      ok:
        latestState !== undefined &&
        latestState.boundary === "local_observability_only" &&
    ...
    },
    {
      ...
      ok:
        latestState === undefined ||
        (latestMindModel?.boundary === "local_mind_model_only" &&
          ...
          ((latestFlowGraph?.boundary === "local_flow_graph_only" &&
            ...
            flowGraph.ok) &&
          latestContextRecovery?.boundary === "local_context_recovery_exam_only"),
    ...
    },
    {
      ...
      ok:
        ...
        currentTrainingPlan.payload?.boundary === "local_brain_training_plan_only" &&
    ...
    },
    {
      ...
      ok:
        ...
        selfRepairHands.boundary === "local_self_repair_hands_only" &&
