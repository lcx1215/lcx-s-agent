...
//
// The existing eval (scripts/dev/local-brain-distill-eval.ts) trains/measures
...
// Keep synthetic labels inside the same bounded array contract as real
// distillation rows.  Without this guard the generator could emit a perfectly
// deterministic but impossible-to-pack oracle (or silently teach an
// over-wide answer), which makes the synthetic stream a source of label
// leakage rather than a rule-learning probe.
...
export function deriveLabel(features: TaskFeatures): DerivedLabel {
  ...
  // so this is not a hidden label axis.
  if (features.assetClasses.length > 0) {
  ...
  return {
    ...
    forbiddenModules:
      features.learningRequest && !features.sourceSupplied && features.assetClasses.length === 0
        ...
        : [],
    // Do not silently truncate these arrays. A synthetic case whose complete
    // contract cannot fit the production cap is rejected by the stream
    // builder, rather than converted into a contradictory training label.
...
function renderAsk(features: TaskFeatures, rng: () => number): string {
  ...
  if (assets.length > 0) {
    parts.push(
      features.tradeWording
        ? pick(rng, ["，research-only，不要交易建议", "，只做研究不要下单", "，不要给买卖点"])
        : pick(rng, ["，research-only 研究分析", "，只做研究分析", "，先做证据审查"]),
    );
  }
  // `dataSupplied` is a label-relevant axis.  Make it visible in the natural
  // language so the same ask cannot receive two contradictory oracle labels
  // merely because the generator kept the data state hidden.
...
//
// The seeded stream varies feature combinations and surface paraphrases. The
// feature space is finite, so exact prompt/completion duplicates can still
// occur at the slice boundary; the dataset owner deduplicates those pairs and
// records the skipped count instead of pretending they are novel evidence.
// That bounded variation is the point of feeding this into training rather
// than relying on the fixed bank alone.
// ---------------------------------------------------------------------------
