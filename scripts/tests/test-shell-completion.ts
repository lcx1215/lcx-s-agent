 ...
 * `lcx update`. Use it to verify the completion installation flow
...
\`lcx update\`, \`lcx doctor\`, and \`lcx onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# LCX Agent Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`lcx completion --install\` later to enable.`));
    return;
  }
